  ...
  <div class="page-container bg-white d-flex flex-column">
    ...
    >
      <div>
        ...
        <div class="text-body-2 text-grey mb-3">จัดการใบเสนอราคา, ใบส่งของ, ใบเสร็จรับเงิน</div>
      </div>
      ...
      <v-row dense>
        <v-col cols="12" sm="6" md="4">
          <v-text-field
            ...
            label="ค้นหา (ID, เลขที่ใบเสนอราคา, เลขที่ใบส่งของ, เลขที่ใบเสร็จ, ชื่อลูกค้า)"
    ...
    <div class="flex-grow-1 bg-white">
      ...
      >
        <template v-slot:[`item.id`]="{ item }">
          <span class="font-weight-bold text-grey-darken-4 cursor-pointer">
            {{ item.id }}
          </span>
        </template>

        <template v-slot:[`item.quotationId`]="{ item }">
          <span class="text-body-2 font-weight-medium text-indigo-darken-4">
            {{ item.quotationId || '-' }}
          </span>
        </template>

        <template v-slot:[`item.deliveryNoteNo`]="{ item }">
          <span class="text-body-2 text-deep-purple-darken-1">
            {{ item.deliveryNoteNo || '-' }}
          </span>
        </template>

        <template v-slot:[`item.receiptNo`]="{ item }">
          <span class="text-body-2 text-teal-darken-1">
            {{ item.receiptNo || '-' }}
          </span>
        </template>
        ...
        <template v-slot:[`item.total`]="{ item }">
          <div class="font-weight-bold text-theme-primary">
             {{ item.total }}
          </div>
        </template>
        ...
        <template v-slot:[`item.actions`]="{ item }">
          <v-menu location="bottom end" transition="scale-transition" offset="5">
            ...
            <v-list density="compact" elevation="2" rounded="lg" class="py-1">
              <v-list-item @click="viewDetails(item)" prepend-icon="mdi-eye-outline" value="view" active-color="#4D2FB2">
              ...
              <v-list-item @click="deleteItem(item)" prepend-icon="mdi-trash-can-outline" value="delete">
                <v-list-item-title class="text-red text-body-2">ลบเอกสาร</v-list-item-title>
              </v-list-item>
            </v-list>
          </v-menu>
        </template>
        ...
        <template v-slot:no-data>
          <div class="pa-10 text-center">
            <v-avatar color="grey-lighten-4" size="80" class="mb-4">
              <v-icon icon="mdi-file-document-outline" size="40" color="grey-lighten-1"></v-icon>
            </v-avatar>
            <h3 class="text-h6 text-grey-darken-1 font-weight-bold mb-1">ไม่พบข้อมูลเอกสาร</h3>
            <p class="text-body-2 text-grey mb-4">ลองปรับตัวกรองค้นหา หรือสร้างเอกสารใหม่</p>
            <v-btn
              color="#4D2FB2"
              variant="flat"
              rounded="pill"
  ...
  methods: {
        ...
        if (res.data.message === "success") {
            ...
            const mappedData = rawData.map(item => {
                    ...
                    item.productSections.forEach(section => {
                        if (section && section.items && Array.isArray(section.items)) {
                            section.items.forEach(p => {
                                ...
                                const price = parseFloat(p.unit_price) || parseFloat(p.unitPrice) || 0;
                                total += (qty * price);
                            });
                        }
                    });
                ...
                return {
                    ...
                    customerName: item.customer_name || item.customerName || 'ไม่ระบุ',
                    total: grandTotal.toLocaleString('th-TH', { style: 'currency', currency: 'THB' }),
                    status: item.current_status || item.currentStatus || 'QUOTATION',
                    
                    deliveryNoteNo: item.delivery_note_no || item.deliveryNoteNo || '',
                    receiptNo: item.receipt_no || item.receiptNo || '',
                    
                    issueDate: item.issue_date_str || item.issueDateStr || item.issue_date || '-',
                    fullData: item
                };
            });
    ...
    getStatusColor(status) {
      ...
      switch (status) {
        ...
        case "DELIVERY_NOTE": return "info";
    ...
    viewDetails(item) {
      if (item && item.id) {
        this.$router.push({ name: "TheQuotationDetail", params: { id: item.id } });
      }
    },
      ...
      this.fp = flatpickr(el, {
        ...
        onChange: (selectedDates) => {
          if (selectedDates.length === 2) {
            ...
            const startStr = this.formatToThaiDate(start); 
            const endStr = this.formatToThaiDate(end);
